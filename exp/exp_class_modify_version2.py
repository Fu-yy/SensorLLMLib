# exp/exp_sensorllm_unified.py  (V2 = V1 + optimizer param groups + cosine scheduler)
import os
import time
import json
import yaml
...
class Exp_Classification(Exp_Basic):
    def __init__(self, args):
        ...
        if not hasattr(self.args, "weight_decay"):
            setattr(self.args, "weight_decay", 0.01)
        if not hasattr(self.args, "min_lr"):
            setattr(self.args, "min_lr", 1e-6)
    ...
    def _to_bool_mask(self, padding_mask: torch.Tensor) -> torch.Tensor:
    ...
    def _select_optimizer(self):
        ...
        for n, p in self.model.named_parameters():
            if not p.requires_grad:
                continue
            nn_ = n.lower()
            if n.endswith("bias") or ("norm" in nn_) or ("embedding" in nn_) or ("embed" in nn_):
                no_decay.append(p)
            else:
                decay.append(p)
        ...
        return optim.RAdam(groups, lr=lr)
    ...
    def pretrain(self, setting):
        ...
        scheduler = CosineAnnealingLR(opt, T_max=max(1, self.args.train_epochs), eta_min=float(getattr(self.args, "min_lr", 1e-6)))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.log(f"[Stage1-Pretrain] epoch={epoch+1} lr={cur_lr:.2e} train_mse={train_mse:.6f} val_mse={val_mse:.6f}")
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.log(f"[Stage2-Classify] Epoch:{epoch + 1} | lr:{cur_lr:.2e} | Train:{train_loss:.4f} | "
