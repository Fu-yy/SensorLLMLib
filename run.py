...
if __name__ == '__main__':
    ...
    # datasets loader
    parser.add_argument('--datasets', type=str, required=True, default='ETTh1', help='dataset type')
    ...
    parser.add_argument('--trainable_modules', type=str, default="sensor_patch_proj,channel_id,patch_pos,mask_embed,recon_head,cls_head,pool_query,pool_attn", help='trainable_modules')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.two_stage == 0:
                print('>>>>>>>start training stage2 : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
                ...
                if args.stage == 1:
                    print('>>>>>>>start training stage1 : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
