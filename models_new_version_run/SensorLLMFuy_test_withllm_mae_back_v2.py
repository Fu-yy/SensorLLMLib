...
# ============================================================
@torch.no_grad()
def sanity_check_prompt_alignment(online, prompt: str, topk: int = 30):
    tok = online.tokenizer
    model = online.model
    dev = online._input_device()

    # ensure last position is a "generation start"
    ...
    cand_ids = online.cand_ids_cpu.to(dev)  # [K]
    cand_probs = probs.index_select(0, cand_ids)

    print("=== Candidate mass ===")
    print("cand sum:", float(cand_probs.sum().item()))
    print("cand max:", float(cand_probs.max().item()), "argmax:", int(cand_probs.argmax().item()))
...
class OnlineLLMTeacher:
    def __init__(
        ...
        # candidates: two-digit tokens
        self.candidates = [f"{k:02d}" for k in range(self.K)]

        # must be SINGLE token (you already verified)
        cand_ids: List[int] = []
        for c in self.candidates:
            ids = self.tokenizer.encode(c, add_special_tokens=False)
            if len(ids) != 1:
                raise ValueError(f"Candidate '{c}' not single-token: ids={ids}")
            cand_ids.append(ids[0])
        self.cand_ids_cpu = torch.tensor(cand_ids, dtype=torch.long, device="cpu")  # [K]

    def _input_device(self) -> torch.device:
        return self.model.get_input_embeddings().weight.device

    @staticmethod
    def _ensure_trailing_space(prompts: List[str]) -> List[str]:
        return [p if (p and p.endswith(" ")) else (p + " ") for p in prompts]

    # -------------------------
    # Mode 1: next-token gather (FAST)
    # -------------------------
    @torch.no_grad()
    def probs_next_batch(self, prompts: List[str], temperature: float = 1.0) -> torch.Tensor:
        """
        return [B,K] on CPU
        """
        ...
        cand_ids = self.cand_ids_cpu.to(last_logits.device)
    ...
    @torch.no_grad()
    def probs_next(self, prompt: str, temperature: float = 1.0) -> torch.Tensor:
        return self.probs_next_batch([prompt], temperature=temperature)[0]

    # -------------------------
    # Mode 2: continuation logprob (ROBUST / DEBUG)
    # (Here each candidate is a SINGLE token too, so cont is trivial: logp(candidate | prompt))
    # -------------------------
    @torch.no_grad()
    def probs_cont_batch(self, prompts: List[str], temperature: float = 1.0) -> torch.Tensor:
        ...
        logp_all = F.log_softmax(logits / temp, dim=-1)  # [BK,T,V]

        # We want logp of the LAST token (candidate token) given prefix.
        # Find last valid position for each row: last token index = sum(attn_mask)-1
        last_pos = attn_mask.sum(dim=1) - 1  # [BK]
        # The probability of token at position t comes from logits at t-1
        pred_pos = (last_pos - 1).clamp_min(0)  # [BK]

        target_ids = input_ids[torch.arange(input_ids.size(0), device=dev), last_pos]  # [BK]
        token_logp = logp_all[torch.arange(input_ids.size(0), device=dev), pred_pos, :].gather(
            -1, target_ids.unsqueeze(-1)
        ).squeeze(-1)  # [BK]

        B = len(prompts)
        scores = token_logp.view(B, self.K).float().cpu()  # [B,K] on CPU
...
class TeacherProvider:
    ...
    def _online_probs_batch(self, prompts: List[str]) -> torch.Tensor:
        if self.score_mode == "next":
            return self.online.probs_next_batch(prompts, temperature=self.temperature)  # [B,K] CPU
        else:
            return self.online.probs_cont_batch(prompts, temperature=self.temperature)  # [B,K] CPU

    def _online_probs(self, prompt: str) -> torch.Tensor:
        if self.score_mode == "next":
            return self.online.probs_next(prompt, temperature=self.temperature)  # [K] CPU
        else:
            return self.online.probs_cont(prompt, temperature=self.temperature)  # [K] CPU
    ...
    def get_teacher_probs(self, pid_row_cpu: torch.Tensor, s: int, e: int) -> torch.Tensor:
        ...
        if self.debug and self.online is not None:
            sanity_check_prompt_alignment(self.online, prompt, topk=30)
    ...
    @torch.no_grad()
    def get_teacher_probs_batch(self, pid_cpu: torch.Tensor, span_info: List[Tuple[int, int, int]]) -> torch.Tensor:
        """
        pid_cpu: [B,P] on CPU
        span_info: list of (s,e,k) length B
        return: [B,K] on CPU float32
        """
        B = pid_cpu.size(0)
        out = [None] * B
        ...
        for b in range(B):
            ...
            tokens = pid_to_tokens(pid_cpu[b])
            prompt = build_llm_prompt(tokens, s, e, K=self.K)
            miss_idx.append(b)
            miss_prompts.append(prompt)
            miss_keys.append(key)
        print(self.online.tokenizer.encode("00", add_special_tokens=False))
        print(self.online.tokenizer.encode(" 00", add_special_tokens=False))
        if len(miss_idx) > 0:
            if self.debug and self.online is not None:
                sanity_check_prompt_alignment(self.online, miss_prompts[0], topk=30)

            probs_bk = self._online_probs_batch(miss_prompts)  # [Bm,K] CPU
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if self.warmup_only != 1:
            ...
            self.missing_head = nn.Linear(self.resampler_dim, self.K)
    ...
    def enable_kd(self):
        ...
        if self.missing_head is None:
            self.missing_head = nn.Linear(self.resampler_dim, self.K).to(self.device)
    ...
    def forward(self, batch_x, padding_mask=None, mode: Optional[str] = None, labels=None):
        ...
        p_llm_cpu = self.teacher.get_teacher_probs_batch(pid_cpu, span_info)  # [B,K] CPU
        p_llm = p_llm_cpu.to(x_norm.device)
