...
class Model(nn.Module):
    ...
    def forward(self, x_imu, padding_mask=None, mode=None, labels=None):
        ...
        if self.stage == 1:
            ...
            # Student Forward
            student_logits, feat,emb ,pred_len,label_res= self.student(x_pad, mask_bool)  # [B, P, K]
            ...
            # Loss Calculation
            # 1. 拿到原始数据中被 Mask 的部分
            target_pixels = label_res[final_mask] # 需要先把原始 x 变成 patch 形状 [B, P, patch_len * C]
            # 2. 拿到预测数据中被 Mask 的部分
            ...
            loss_total = self.args.loss_recon_weight* loss_recon +self.args.loss_distill_weight* self.lambda_distill * loss_distill +self.args.loss_recon_teacher_weight*loss_recon_teacher + self.args.loss_mse_student_weight * loss_mse_student
            # meta = {
        ...
        # ====================
            ...
            # Student Forward
            _, feat,_,_,_ = self.student(x_pad, mask_bool=None)  # [B, P, D]
