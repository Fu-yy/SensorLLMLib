...
class AlignmentModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        # ============================================================
        # 4.5.1 Low-motion Gated Posture-Sensitive Expert
        # ============================================================
        # This expert is designed for cross-dataset posture-sensitive
        # or low-motion activities, not only UCIHAR Sitting/Standing/Laying.
        #
        # It uses channel-level physical statistics:
        #   mean/std/min/max/energy/abs_mean/slope
        # and produces logits only for posture-sensitive classes.
        self.use_posture_branch = bool(int(getattr(args, "use_posture_branch", 1)))

        # Backward-compatible alias: if you still use --use_static_branch
        if hasattr(args, "use_static_branch"):
            self.use_posture_branch = bool(int(getattr(args, "use_static_branch", 1)))

        self.posture_alpha = float(
            getattr(args, "posture_alpha", getattr(args, "static_alpha", 0.8))
        )

        self.lambda_posture_aux = float(
            getattr(args, "lambda_posture_aux", getattr(args, "lambda_static_aux", 0.2))
        )

        self.lambda_posture_gate = float(
            getattr(args, "lambda_posture_gate", getattr(args, "lambda_static_gate", 0.05))
        )

        # channel_stats: [B, C, 7]
        # gate features:
        #   avg_energy, avg_abs, avg_abs_slope, low_motion_score,
        #   acc_energy, gyro_energy
        self.posture_feature_dim = self.C * self.channel_stat_dim + 6

        self.posture_feature_proj = nn.Sequential(
            nn.LayerNorm(self.posture_feature_dim),
            nn.Linear(self.posture_feature_dim, self.llm_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(self.llm_hidden, self.llm_hidden),
            nn.GELU(),
        ...
        static_class_mask, posture_class_mask = self._build_posture_class_masks(args)
        ...
        self.static_class_indices = torch.where(static_class_mask)[0].cpu().tolist()
        self.posture_class_indices = torch.where(posture_class_mask)[0].cpu().tolist()

        print(
            f"[PrimitiveAlignHAR] use_posture_branch={self.use_posture_branch}, "
            f"posture_alpha={self.posture_alpha}, "
            f"lambda_posture_aux={self.lambda_posture_aux}, "
            f"lambda_posture_gate={self.lambda_posture_gate}"
        )

        print(
            f"[PrimitiveAlignHAR] static_class_indices={self.static_class_indices}, "
            f"static_class_names={[self.label_names[i] for i in self.static_class_indices]}"
        )

        print(
            f"[PrimitiveAlignHAR] posture_class_indices={self.posture_class_indices}, "
            f"posture_class_names={[self.label_names[i] for i in self.posture_class_indices]}"
        )
    ...
    def _parse_label_index_list(self, value):
        """
        Parse label indices from yaml/args.
    ...
    def _build_posture_class_masks(self, args):
        """
        Build two class masks:

        static_class_mask:
            Strictly static classes, e.g., sitting, standing, lying, sleep.

        posture_class_mask:
            Broader posture-sensitive / low-motion / quasi-static classes.
            The posture expert is only allowed to modify these classes.

        Priority:
            1. Explicit args: --static_labels / --posture_sensitive_labels
            2. Dataset yaml: static_labels / posture_sensitive_labels
            3. Keyword fallback from label names
        """
        static_class_mask = torch.zeros(self.num_class, dtype=torch.bool)
        posture_class_mask = torch.zeros(self.num_class, dtype=torch.bool)

        # ------------------------------------------------------------
        # 1. Prefer explicit dataset config or command-line args
        # ------------------------------------------------------------
        cfg_static = None
        cfg_posture = None

        if isinstance(getattr(self, "ds_cfg", None), dict):
            cfg_static = self.ds_cfg.get("static_labels", None)
            cfg_posture = self.ds_cfg.get("posture_sensitive_labels", None)

        arg_static = getattr(args, "static_labels", None)
        arg_posture = getattr(args, "posture_sensitive_labels", None)

        static_ids = self._parse_label_index_list(arg_static)
        if static_ids is None:
            static_ids = self._parse_label_index_list(cfg_static)

        posture_ids = self._parse_label_index_list(arg_posture)
        if posture_ids is None:
            posture_ids = self._parse_label_index_list(cfg_posture)

        if static_ids is not None:
            for i in static_ids:
                if 0 <= i < self.num_class:
                    static_class_mask[i] = True

        if posture_ids is not None:
            for i in posture_ids:
                if 0 <= i < self.num_class:
                    posture_class_mask[i] = True

        # ------------------------------------------------------------
        # 2. Fallback: keyword-based automatic detection
        # ------------------------------------------------------------
        if not posture_class_mask.any():
            static_keywords = [
                "sit",
                "sitting",
                "stand",
            ...
            posture_sensitive_keywords = [
                ...
                # posture-sensitive or quasi-static activities
                "waist",
                "bend",
                "bends",
                "crouch",
            ...
            for i, name in enumerate(self.label_names):
                ...
                # remove leading numbering such as "1. Walking Forward"
                if "." in lname[:4]:
                    lname = lname.split(".", 1)[1].strip()

                is_static = any(k in lname for k in static_keywords)
                is_posture = any(k in lname for k in posture_sensitive_keywords)
                is_dynamic = any(k in lname for k in dynamic_exclusion_keywords)

                if is_static:
                    static_class_mask[i] = True

                # Avoid treating "walking upstairs" as posture-sensitive.
                # Strict static labels are always kept.
                if is_posture and (not is_dynamic or is_static):
                    posture_class_mask[i] = True

        # ------------------------------------------------------------
        # 3. Static classes must be included in posture-sensitive classes
        # ------------------------------------------------------------
        posture_class_mask = posture_class_mask | static_class_mask
        ...
        if not posture_class_mask.any():
            print(
                "[PrimitiveAlignHAR][warn] No static/posture-sensitive labels found. "
                "Posture branch will be effectively disabled."
            )

        return static_class_mask, posture_class_mask

    def _compute_posture_branch(self, channel_stats, segment_stats=None):
        """
        Low-motion gated posture-sensitive expert.
        ...
        segment_stats:
            [B, P, 3*C+4], optional.
            Currently not required, but kept for future extension.

        Returns:
            posture_logits: [B, num_class]
                Only posture-sensitive class logits are non-zero.

            posture_gate: [B, 1]
                Low-motion gate. Larger value means stronger posture expert.

            gate_features: [B, 6]
                avg_energy, avg_abs, avg_abs_slope, low_motion_score,
                acc_energy, gyro_energy.
        """
        B, C, D = channel_stats.shape

        if D != self.channel_stat_dim:
            raise RuntimeError(
                f"channel_stats dim mismatch: got {D}, expected {self.channel_stat_dim}"
            )

        # ------------------------------------------------------------
        # 1. Flatten channel statistics
        # ------------------------------------------------------------
        flat_channel = channel_stats.reshape(B, C * D).float()

        # channel_stats indices:
        # 0 mean, 1 std, 2 min, 3 max, 4 energy, 5 abs_mean, 6 slope
        energy = channel_stats[:, :, 4]
        abs_mean = channel_stats[:, :, 5]
        slope = channel_stats[:, :, 6]

        avg_energy = energy.mean(dim=1, keepdim=True)
        avg_abs = abs_mean.mean(dim=1, keepdim=True)
        avg_abs_slope = slope.abs().mean(dim=1, keepdim=True)

        # Lower energy -> larger low-motion score
        low_motion_score = 1.0 / (1.0 + avg_energy)

        # Accelerometer / gyroscope energy.
        # For common HAR datasets:
        #   first 3 channels are usually accelerometer,
        #   next 3 channels are usually gyroscope.
        # If unavailable, use safe fallbacks.
        if self.C >= 3:
            acc_energy = energy[:, :3].mean(dim=1, keepdim=True)
        else:
            acc_energy = avg_energy

        if self.C >= 6:
            gyro_energy = energy[:, 3:6].mean(dim=1, keepdim=True)
        else:
            gyro_energy = energy.new_zeros(B, 1)
        ...
        # ------------------------------------------------------------
        # 2. Posture-sensitive expert logits
        # ------------------------------------------------------------
        posture_input = torch.cat(
            [flat_channel, gate_features],
            dim=-1,
        )

        posture_hidden = self.posture_feature_proj(posture_input)
        raw_posture_logits = self.posture_head(posture_hidden)

        # ------------------------------------------------------------
        # 3. Only allow this expert to affect posture-sensitive classes
        # ------------------------------------------------------------
        if self.posture_class_mask.any():
            posture_logits = raw_posture_logits.new_zeros(raw_posture_logits.shape)
            posture_logits[:, self.posture_class_mask] = raw_posture_logits[:, self.posture_class_mask]
        else:
            posture_logits = raw_posture_logits.new_zeros(raw_posture_logits.shape)
        ...
        # 4. Low-motion gate
        # ------------------------------------------------------------
        posture_gate = self.posture_gate(gate_features)

        return posture_logits, posture_gate, gate_features
    ...
    def forward(self, x_imu, padding_mask=None, mode=None, labels=None, **kwargs):
        ...
        # ------------------------------------------------------------
        # Main primitive-LLM classifier
        # ------------------------------------------------------------
        base_logits = self.activity_head(cls_h)

        # ------------------------------------------------------------
        # Low-motion Gated Posture-Sensitive Expert
        # ------------------------------------------------------------
        if self.use_posture_branch:
            posture_logits, posture_gate, posture_gate_features = self._compute_posture_branch(
                channel_stats=channel_stats,
                segment_stats=segment_stats,
            )

            activity_logits = base_logits + self.posture_alpha * posture_gate * posture_logits
        else:
            posture_logits = base_logits.new_zeros(base_logits.shape)
            posture_gate = base_logits.new_zeros(base_logits.shape[0], 1)
            posture_gate_features = base_logits.new_zeros(base_logits.shape[0], 6)
            activity_logits = base_logits
        ...
        # ------------------------------------------------------------
        # Posture auxiliary loss:
        # Only supervise the posture expert on posture-sensitive labels.
        # This avoids forcing the posture expert to model dynamic activities.
        # ------------------------------------------------------------
        if (
            self.use_posture_branch
            and self.posture_class_mask.any()
            and labels is not None
        ):
            is_posture_label = self.posture_class_mask[labels]

            if is_posture_label.any():
                posture_indices = torch.where(self.posture_class_mask)[0].to(self.device)

                label_to_local = torch.full(
                    (self.num_class,),
                    -1,
                    device=self.device,
                    dtype=torch.long,
                ...
                label_to_local[posture_indices] = torch.arange(
                    posture_indices.numel(),
                    device=self.device,
                    dtype=torch.long,
                )

                local_posture_targets = label_to_local[labels[is_posture_label]]

                posture_aux_logits = posture_logits[is_posture_label][:, posture_indices]

                loss_posture_aux = F.cross_entropy(
                    posture_aux_logits,
                    local_posture_targets,
                )
            else:
                loss_posture_aux = activity_logits.sum() * 0.0
        else:
            loss_posture_aux = activity_logits.sum() * 0.0

        # ------------------------------------------------------------
        # Gate supervision:
        #   strict static class: gate target = 1.0
        #   posture-sensitive but not strict static: gate target = 0.6
        #   dynamic class: gate target = 0.0
        # ------------------------------------------------------------
        if (
            self.use_posture_branch
            and self.posture_class_mask.any()
            ...
            is_static = self.static_class_mask[labels].unsqueeze(-1)
            is_posture = self.posture_class_mask[labels].unsqueeze(-1)

            gate_target = torch.where(
                is_posture,
                torch.full_like(gate_target, 0.6),
                gate_target,
            )
            ...
            loss_posture_gate = F.binary_cross_entropy(
                posture_gate.clamp(1e-6, 1.0 - 1e-6),
                gate_target,
            )
        else:
            loss_posture_gate = activity_logits.sum() * 0.0
        ...
        loss_total = (
                ...
                + self.lambda_posture_aux * loss_posture_aux
                + self.lambda_posture_gate * loss_posture_gate
        )
        ...
        with torch.no_grad():
            ...
            if self.use_posture_branch and self.posture_class_mask.any():
                is_static_label_eval = self.static_class_mask[labels]
                is_posture_label_eval = self.posture_class_mask[labels]
                is_dynamic_label_eval = ~is_posture_label_eval

                if is_posture_label_eval.any():
                    posture_pred = posture_logits.argmax(dim=-1)
                    posture_branch_acc = (
                            posture_pred[is_posture_label_eval]
                            == labels[is_posture_label_eval]
                    ).float().mean()
                else:
                    posture_branch_acc = activity_logits.new_tensor(0.0)

                posture_gate_mean = posture_gate.mean()

                gate_static = (
                    posture_gate[is_static_label_eval].mean()
                    if is_static_label_eval.any()
                    else activity_logits.new_tensor(0.0)
                )

                posture_only_mask = is_posture_label_eval & (~is_static_label_eval)

                gate_posture = (
                    posture_gate[posture_only_mask].mean()
                    if posture_only_mask.any()
                    else activity_logits.new_tensor(0.0)
                )

                gate_dynamic = (
                    posture_gate[is_dynamic_label_eval].mean()
                    if is_dynamic_label_eval.any()
                    else activity_logits.new_tensor(0.0)
                )
            else:
                posture_branch_acc = activity_logits.new_tensor(0.0)
                posture_gate_mean = activity_logits.new_tensor(0.0)
                gate_static = activity_logits.new_tensor(0.0)
                gate_posture = activity_logits.new_tensor(0.0)
                gate_dynamic = activity_logits.new_tensor(0.0)
        ...
        metrics = {
            ...
            "loss_posture_aux": float(loss_posture_aux.detach().item()),
            "loss_posture_gate": float(loss_posture_gate.detach().item()),
            ...
            "posture_branch_acc": float(posture_branch_acc.detach().item()),
            "posture_gate_mean": float(posture_gate_mean.detach().item()),
            "gate_static": float(gate_static.detach().item()),
            "gate_posture": float(gate_posture.detach().item()),
            "gate_dynamic": float(gate_dynamic.detach().item()),
