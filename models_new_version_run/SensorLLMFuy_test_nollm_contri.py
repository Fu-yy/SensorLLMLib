...
def aug_channel_dropout(x, drop_p=0.2):
    B,L,C = x.shape
    m = (torch.rand(B,1,C, device=x.device) > drop_p).type_as(x)
    return x * m

def aug_freq_mask(x, keep_ratio=0.8):
    # simple: rfft -> mask high freq bands -> irfft
    xf = torch.fft.rfft(x.float(), dim=1)        # [B,F,C]
    F = xf.size(1)
    k = int(F * keep_ratio)
    mask = torch.zeros_like(xf)
    mask[:, :k, :] = 1
    x_rec = torch.fft.irfft(xf * mask, n=x.size(1), dim=1)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # ---- extra aug knobs (contrastive) ----
        self.max_shift = int(getattr(args, "max_shift", max(1, self.seq_len // 50)))  # e.g. seq_len=200 -> 4
        self.scale_min = float(getattr(args, "scale_min", 0.8))
        self.scale_max = float(getattr(args, "scale_max", 1.25))
        self.chan_drop = float(getattr(args, "chan_drop", 0.2))
        self.freq_keep = float(getattr(args, "freq_keep", 0.8))

        # how many extra augs to apply per view (besides mask+noise)
        self.aug_k_min = int(getattr(args, "aug_k_min", 2))
        self.aug_k_max = int(getattr(args, "aug_k_max", 3))
    ...
    def _time_shift(self, x: torch.Tensor, max_shift: int) -> torch.Tensor:
        # x: [B, L, C]
        if max_shift <= 0:
            return x
    ...
    def _channel_dropout(self, x: torch.Tensor, drop_p: float) -> torch.Tensor:
        # drop whole channels for a sample (like sensor missing)
        if drop_p <= 0:
            return x
        B, L, C = x.shape
        keep = (torch.rand(B, 1, C, device=x.device) > drop_p).type_as(x)
        return x * keep

    def _time_scaling(self, x: torch.Tensor, scale_min: float, scale_max: float) -> torch.Tensor:
        """
        Random time scaling via linear interpolation.
        - sample a scale s ~ U[scale_min, scale_max]
        - resample to L' = round(L * s)
        - center-crop or pad back to L
        """
        if scale_max <= 0 or scale_min <= 0 or abs(scale_max - 1.0) < 1e-6 and abs(scale_min - 1.0) < 1e-6:
            return x

        B, L, C = x.shape
        s = float(torch.empty(1, device=x.device).uniform_(scale_min, scale_max).item())
        L2 = max(2, int(round(L * s)))

        # interpolate expects [B,C,L]
        y = x.transpose(1, 2)  # [B,C,L]
        y = F.interpolate(y, size=L2, mode="linear", align_corners=False)
        y = y.transpose(1, 2)  # [B,L2,C]
        ...
        if L2 > L:
            ...
            pad_left = (L - L2) // 2
            pad_right = L - L2 - pad_left
            y = F.pad(y, (0, 0, pad_left, pad_right))  # pad on time dim
        return y

    def _freq_mask(self, x: torch.Tensor, keep_ratio: float) -> torch.Tensor:
        """
        Simple low-pass style freq masking:
        - rfft over time dim
        - keep first k freq bins, zero the rest
        - irfft back
        """
        ...
        xf = torch.fft.rfft(x.float(), dim=1)  # [B, F, C]
        Fbin = xf.size(1)
        k = max(1, int(round(Fbin * keep_ratio)))
        mask = torch.zeros_like(xf)
        mask[:, :k, :] = 1
        y = torch.fft.irfft(xf * mask, n=L, dim=1)
        return y.to(dtype=x.dtype)

    def _patch_zero_mask(self, x: torch.Tensor, mask_rate: float) -> torch.Tensor:
        B, L, C = x.shape
        P = self.P
        patch_mask = (torch.rand(B, P, device=x.device) < mask_rate)  # [B,P] bool
        patches = x.view(B, P, self.patch_len, C)
        m = patch_mask.view(B, P, 1, 1)
        patches = torch.where(m, torch.zeros_like(patches), patches)
        return patches.view(B, L, C)

    def _make_view(self, x_norm: torch.Tensor, mask_rate: float, noise_std: float) -> torch.Tensor:
        """
        Upgraded contrastive augmentation:
        - apply 2~3 random augs among {time_shift, time_scaling, channel_dropout, freq_mask}
        - then apply your original patch mask + gaussian noise
        """
        x = x_norm

        # ---- choose K augs to apply (at least 2) ----
        # augs are functions that map x -> x
        aug_fns = [
            lambda t: self._time_shift(t, self.max_shift),
            lambda t: self._time_scaling(t, self.scale_min, self.scale_max),
            lambda t: self._channel_dropout(t, self.chan_drop),
            lambda t: self._freq_mask(t, self.freq_keep),
        ]

        kmin = max(0, int(getattr(self, "aug_k_min", 2)))
        kmax = max(kmin, int(getattr(self, "aug_k_max", 3)))
        k = int(torch.randint(kmin, kmax + 1, (1,), device=x.device).item()) if len(aug_fns) > 0 else 0

        if k > 0:
            idx = torch.randperm(len(aug_fns), device=x.device)[:k].tolist()
            for i in idx:
                x = aug_fns[i](x)

        # ---- keep your original patch mask + noise ----
        x = self._patch_zero_mask(x, mask_rate)
