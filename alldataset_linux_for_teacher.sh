...
WARMUP_ONLY=1
KMEANS_TRIGGER_EPOCH=5
BUILD_KMEANS_CENTERS=1
TEACHER_MODE='online'
for i in 1;do
...
python -u run.py \
  ...
  --num_workers 10 --warmup_only $WARMUP_ONLY --kmeans_trigger_epoch $KMEANS_TRIGGER_EPOCH --build_kmeans_centers $BUILD_KMEANS_CENTERS --teacher_mode $TEACHER_MODE \
...
DATA_ROOT="/root/autodl-tmp/SensorLLMLib/datasets/pamap2+physical+activity+monitoring/PAMAP2_Dataset/PAMAP2_Dataset"
DATA_KEY="pamap50"
DATA_NAME="PAMAP50"
...
DATA_ROOT="/root/autodl-tmp/SensorLLMLib\datasets\WISDM_ar_latest\WISDM_ar_v1.1"   # <- 改成你的路径(文件夹内有 WISDM_ar_v1.1_raw.txt) 或直接指向 raw.txt
DATA_KEY="wisdm"
DATA_NAME="WISDM"
...
DATA_ROOT="/root/autodl-tmp/SensorLLMLib/datasets/heterogeneity+activity+recognition/Activity recognition exp/Activity recognition exp"   # <- 改成你的路径(含 Phones_*.csv / Watch_*.csv)
DATA_KEY="hhar"
...
#DATA_ROOT="/root/autodl-tmp/SensorLLMLib/datasets/heterogeneity+activity+recognition/Activity recognition exp/Activity recognition exp"   # <- 同上
#DATA_KEY="hhar"
...
DATA_ROOT="/root/autodl-tmp/SensorLLMLib\datasets\motion-sense-master\motion-sense-master\data"  # <- 改成你的路径
