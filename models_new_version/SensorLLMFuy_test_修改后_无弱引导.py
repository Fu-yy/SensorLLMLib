...
class PatchEmbedding(nn.Module):
    ...
    Channel Mixing Embedding.
    Input: [B, Seq_Len, C] -> Patch: [B, Num_Patches, Embed_Dim]
    保留了时间窗口内所有通道的联合特征 (Correlation)。
    """
    ...
    def __init__(self, seq_len: int, patch_len: int, in_channels: int, embed_dim: int):
        ...
        # 核心映射：将 (Patch_len * Channels) 视为一个特征单元
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, L, C]
        B, L, C = x.shape
        # Patchify & Flatten: [B, Num_Patches, Patch_Len * C]
...
# ============================================================
# 2. Main Model (Drop-in Replacement)
# ============================================================
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # -------- view augmentation knobs --------
        self.mask_rate1 = float(getattr(args, "mask_rate1", 0.5))
        self.mask_rate2 = float(getattr(args, "mask_rate2", 0.7))
        ...
        # Resampler 内部维度，建议小于 LLM 维度以节省计算
        ...
        # 3. Projector (Bridge to LLM)
        self.llm_proj = nn.Linear(self.resampler_dim, self.llm_hidden_size)

        # 4. Heads
        # Stage 1: Contrastive Head (on Resampler output directly for efficiency)
        ...
        # Stage 2: Classification Head (on LLM output)
    ...
    def _make_view(self, x_norm: torch.Tensor, mask_rate: float, noise_std: float) -> torch.Tensor:
        """
        数据增强：Masking + Gaussian Noise
        """
        B, L, C = x_norm.shape
        # Patch-level masking logic
        P = self.P
        patch_mask = (torch.rand(B, P, device=x_norm.device) < mask_rate)

        # Patchify manually for masking
    ...
    def _get_text_embeds(self, B: int, device) -> Tuple[torch.Tensor, torch.Tensor]:
        if self._prompt_cache is None:
            # Prompt 设计：简单明了，作为 LLM 的 Context
            self._prompt_cache = (
                f"Analyze the following {self.dataset_key} sensor sequence. "
                "The features are extracted from accelerometers and gyroscopes. "
                "Classify the human activity."
            )
    ...
    def _infonce_loss(self, y1: torch.Tensor, y2: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        return loss, {"B": float(B), "loss": loss.item(), "acc": acc}
    ...
    def forward(self, batch_x, padding_mask=None, mode: Optional[str] = None, labels=None):
        ...
        if mode == "classify":
            # A. Encode Sensor
            # [B, P, D_res]
            sensor_feats = self.patch_embed(x_norm)

            # [B, M, D_res] -> Deep Resampler Bottleneck
            sensor_latents = self.resampler(sensor_feats)

            # [B, M, D_llm] -> Project to LLM
            inputs_sensor = self.llm_proj(sensor_latents)

            # B. Get Text Prompt
            # [B, T, D_llm]
            text_embeds, text_mask = self._get_text_embeds(x.shape[0], x.device)
            text_embeds = text_embeds.to(dtype=inputs_sensor.dtype)

            # C. Concat: [Text, Sensor] -> LLM
            inputs_embeds = torch.cat([text_embeds, inputs_sensor], dim=1)

            # Mask: Text部分真实，Sensor部分全1 (Allow attend to all latents)
            B, M = inputs_sensor.shape[:2]
            sensor_mask = torch.ones(B, M, device=x.device, dtype=text_mask.dtype)
            attention_mask = torch.cat([text_mask, sensor_mask], dim=1)

            # D. LLM Forward
            # with torch.no_grad():  # LLM frozen
            # 只有 inputs_embeds 带梯度，LLM 内部参数不更新
            outputs = self.llm(
            ...
            last_hidden = outputs.hidden_states[-1]  # [B, T+M, D_llm]

            # E. Pooling & Classification
            # 只取 Sensor Latents 对应的部分 [B, M, D_llm] (即最后 M 个 token)
            sensor_out_llm = last_hidden[:, -M:, :]

            # Mean Pooling over Latents
            pooled = sensor_out_llm.mean(dim=1)

            logits = self.cls_head(pooled)
            return logits

        # ---------------------------------------------------
        # Mode: Pretrain (Stage 1 / InfoNCE)
        # ---------------------------------------------------
        # [Scientific Optimization]:
        # 预训练时为了节省显存并增大Batch Size，我们不经过 LLM。
        # 只训练 PatchEmbed -> Resampler -> ProjHead。
        # 目标是让 Resampler 学会提取具有判别性的 Latents。

        # View 1
        v1 = self._make_view(x_norm, self.mask_rate1, self.noise_std1)
        z1 = self.patch_embed(v1)
        lat1 = self.resampler(z1)  # [B, M, D_res]
        # Mean pool latents -> Project
    ...
    def load_wrapper(self, path: str, map_location="cpu"):
        ...
        self.load_state_dict(sd, strict=False)  # strict=False 以防增加参数导致不兼容
