...
# ============================================================
# 2. Main Model (Route A: MAE + LLM)
# ============================================================
class Model(nn.Module):
    """
    Stage 1: Masked Autoencoder (MAE) Pretraining. Sensor -> Resampler -> Decoder -> Recon.
    Stage 2: LLM Classification. Sensor -> Resampler -> Projector -> LLM -> Classify.
    """
    ...
    def __init__(self, args):
        ...
        # 3. Components for Stage 1 (Pretrain)
        # Decoder for MAE (Projects Latents -> Patch)
        ...
        # 4. Components for Stage 2 (Classify)
        # Projector (Resampler -> LLM)
    ...
    def _random_patch_mask(self, B: int, P: int, device) -> torch.Tensor:
        """
        生成 Mask：1 代表 Masked (不可见)，0 代表 Visible
        注意：在 MAE 逻辑中，Resampler 通常只能看到 visible patches。
        但在 Resampler 架构中，我们通常输入 embedding 后用 mask token 替换，或者传入所有。
        这里采用：输入所有，但在 Patch Embedding 后将 masked 部分替换为 Learnable Mask Token (implied)
        或者更简单：直接 mask 掉输入 noise。
        """
        patch_mask = (torch.rand(B, P, device=device) < self.mask_rate)
        # Safety: ensure at least one visible and one masked
        if P >= 2:
    ...
    def forward(self, batch_x, padding_mask=None, mode: Optional[str] = None, labels=None):
        ...
        # Instance Norm (Important for MAE reconstruction stability)
        mu = x_seq.mean(dim=1, keepdim=True)
        sigma = x_seq.std(dim=1, keepdim=True).clamp_min(self.norm_eps)
        x_norm = (x_seq - mu) / sigma

        # ---------------------------------------------
        # Stage 2: Classification (LLM Path)
        # ---------------------------------------------
        if mode == "classify":
            # 1. Encode & Resample
            z = self.patch_embed(x_norm)  # [B, P, D_res]
            lat = self.resampler(z)  # [B, M, D_res]

            # 2. Project to LLM Space
            inputs_sensor = self.llm_proj(lat)  # [B, M, D_llm]
        ...
        # ---------------------------------------------
        # Stage 1: MAE Pretraining (Scientific Path)
        # Bypass LLM completely -> High Efficiency
        # ---------------------------------------------

        # 1. Masking
        # Mask inputs by zeroing out specific patches (Simple MAE)
        B, L, C = x_norm.shape
        patch_mask = self._random_patch_mask(B, self.P, x_norm.device)  # [B, P] boolean
        ...
        x_masked_patches[mask_expanded] = 0.0  # Zero out masked patches

        # Flatten back for embedding (although PatchEmbed handles view, let's keep consistent input)
        x_masked_seq = x_masked_patches.view(B, L, C)

        # 2. Encode Visible+Masked (Zeros) -> Resampler
        z = self.patch_embed(x_masked_seq)
        latents = self.resampler(z)  # [B, M, H] -> Compressed Representation

        # 3. Decode -> Reconstruct All Patches
        # Decoder queries position embeddings and looks at Latents
        pred_patches = self.mae_decoder(latents)  # [B, P, Patch_Len * C]

        # 4. MSE Loss (Only on Masked Patches)
        target_patches = x_patches  # GT
        ...
        loss = loss.mean(dim=-1)  # [B, P] mean over dimensions

        # Only avg loss over masked patches
