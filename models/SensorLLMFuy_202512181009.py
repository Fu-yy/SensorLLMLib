...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        # 0) dataset config: prefer injected ds_cfg, fallback to yaml
        self.dataset_key = str(getattr(args, "dataset_key", getattr(args, "data", "mhealth"))).lower()

        ds_cfg_from_args = getattr(args, "ds_cfg", None)
        if isinstance(ds_cfg_from_args, dict) and len(ds_cfg_from_args) > 0:
            self.ds_cfg = ds_cfg_from_args
        else:
            self.ds_cfg = {}
