...
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_alb_transformer"
...
if [ -n "$TARGET_DIR" ] && [ -d "$TARGET_DIR" ]; then
    find "$TARGET_DIR" -type f -name "*.pth" -delete
...
python -u run.py \
  ...
  --model "$model_name" --data $DATA_NAME --dataset_key $DATA_KEY \
  --seq_len $ALIGN_W_MAX --patch_len $PATCH_LEN --stride $PATCH_LEN --stage 1 \
  --batch_size $BATCH_SIZE --itr $ITR --trainable_modules $PRETRAIN_trainable_modules \
...
python -u run.py \
  ...
  --model "$model_name" --data $DATA_NAME --dataset_key $DATA_KEY \
  --seq_len $SEQ_LEN --patch_len $PATCH_LEN --stride $PATCH_LEN --stage 2 \
  --batch_size $BATCH_SIZE --itr $ITR --trainable_modules $TRAIN_trainable_modules \
...
# 生成一个全局时间标签，这样这一次批量运行的所有日志都在同一个大目录下
GLOBAL_TIME_TAG=$(date +"%Y%m%d_%H%M%S")
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_alb_linear"
mkdir -p "$GLOBAL_LOG_ROOT"
...
# 生成一个全局时间标签，这样这一次批量运行的所有日志都在同一个大目录下
GLOBAL_TIME_TAG=$(date +"%Y%m%d_%H%M%S")
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_alb_prompt"
mkdir -p "$GLOBAL_LOG_ROOT"
...
model_name=SensorLLMFuy_test_withllm_mae_vqvae_alb_prompt
PRETRAIN_trainable_modules="patch_embed,resampler,mae_decoder"
TRAIN_trainable_modules="patch_embed,resampler,llm_proj,cls_head"
# ========================================================
# 2. USC-HAD
