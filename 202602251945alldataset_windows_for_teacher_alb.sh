...
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_mask_70_usehardlabel_1_random"
...
python -u run.py \
  ...
  --num_workers 0 --use_hard_label $use_hard_label --mask_mode $mask_mode --use_hard_label $use_hard_label --mask_mode $mask_mode --mask_rate $MASK_RATE --ts_backbone_yaml $TS_BACNBONE_YAML --vqvae_path $VQVAEPATH \
...
python -u run.py \
  ...
  --num_workers 0 --use_hard_label $use_hard_label --mask_mode $mask_mode --mask_rate $MASK_RATE --ts_backbone_yaml $TS_BACNBONE_YAML --vqvae_path $VQVAEPATH \
...
# 生成一个全局时间标签，这样这一次批量运行的所有日志都在同一个大目录下
GLOBAL_TIME_TAG=$(date +"%Y%m%d_%H%M%S")
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_mask_70_usehardlabel_0_block"
mkdir -p "$GLOBAL_LOG_ROOT"
...
# 生成一个全局时间标签，这样这一次批量运行的所有日志都在同一个大目录下
GLOBAL_TIME_TAG=$(date +"%Y%m%d_%H%M%S")
GLOBAL_LOG_ROOT="./run_log/batch_${GLOBAL_TIME_TAG}_mask_70_usehardlabel_1_channel"
mkdir -p "$GLOBAL_LOG_ROOT"
